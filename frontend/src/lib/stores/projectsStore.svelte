<script context="module" lang="ts">
	...
	export async function updateProject(updatedProject: Project) {
		await graphqlClient.request<UpdateProjectMutation>(UpdateProjectDocument, { ...updatedProject });
		updateItemInList(projects, updatedProject);
	}

	export async function createProject(newProjectName: String, clientId: Number, publicProject: Boolean) {
		const response = await graphqlClient.request<CreateProjectMutation>(CreateProjectDocument, { name: newProjectName, clientId: clientId, public: publicProject });
